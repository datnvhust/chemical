...
function App() {
    ...
    return (
        <div className="App">
            <input onChange={e => setString(e.target.value)} onKeyPress={e => {
                if (e.key === 'Enter') {
                    setBalance(solve(e.target.value))
                }
            }} />
            <div dangerouslySetInnerHTML={createMarkup(string.replace(/([A-Za-z])(\d+)/g, '$1<sub>$2</sub>'))} >
            </div>
